...
enum ToolchainError: Error, CustomStringConvertible {
  ...
  var description: String {
    ...
    case .invalidInstallationArchive(let path):
    ...
    case .invalidResponse(let url, let status, let body):
      var t =
        "Response from \(url) had invalid status \(status) with a body of \(body.count) bytes: "
      t += String(decoding: body, as: UTF8.self)
      return t
    case .unsupportedOperatingSystem:
      return "This version of the operating system is not supported"
    case .noInstallationDirectory(let path):
      return """
...
public class ToolchainSystem {
  ...
  func inferSwiftVersion(
    ...
    guard let version = try fetchLocalSwiftVersion() else {  //, version.contains("wasm") else {
  ...
  public func inferSwiftPath(
    ...
    print("Spec URL: \(String(describing: specURL)) -- \(versionSpec ?? "nil") -- \(swiftVersion)")

    for resolver: any ToolchainResolver in resolvers {
