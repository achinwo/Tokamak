...
public struct JSError: Error {
  ...
  static var undefined: JSError {
    JSError(message: "undefined")
  }
}
...
extension JSPromise {
  public final class PromisePublisher: Publisher {
    ...
    public typealias Failure = JSError
    ...
    fileprivate init(promise: JSPromise) {
      future = .init { resolver in
        promise.then(
          ...
          failure: {
            resolver(.failure(JSError(message: $0.string ?? "Unknown error")))
            return JSValue.undefined
          })
      }
    }
    ...
    where Downstream.Input == JSValue, Downstream.Failure == JSError {
